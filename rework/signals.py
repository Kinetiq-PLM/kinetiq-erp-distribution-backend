...
def handle_failed_shipment_update(sender, instance, created, **kwargs):
    """
    When a FailedShipment is created or updated:
    1. Create a ReworkOrder record immediately
    2. Set rework_type to 'Failed Shipment' automatically
    """
    try:
        # Debug output
        print(f"Processing failed shipment: {instance.failed_shipment_id}")
        
        # Check if this is a new failed shipment or being updated
        # Create rework order regardless of failure reason being empty or not
        with connection.cursor() as cursor:
                ...
                try:
                    with transaction.atomic():
                        ...
                        """, [
                            ...
                            'Failed Shipment'  # Always set for failed shipments
                        ])
                        result = cursor.fetchone()
            ...
            # If there's a failure reason, update the resolution status
...
def handle_rejected_delivery(sender, instance, created, **kwargs):
    """
    ...
    2. Immediately create a ReworkOrder with type 'Rejection'
    """
    try:
        ...
        if instance.receipt_status == 'Rejected':
            ...
            # Now create a ReworkOrder for this rejection if one doesn't already exist
            if rejection_id:
                with connection.cursor() as cursor:
                    cursor.execute("""
                        SELECT rework_id
                        ...
                        try:
                            with transaction.atomic():
                                ...
                                """, [
                                    ...
                                    'Rejection'  # Always set for rejections
                                ])
                                
                                result = cursor.fetchone()
                                rework_id = result[0] if result else None
                                print(f"Created ReworkOrder {rework_id} for Rejection {rejection_id}")
                            ...
                            print(f"Error creating ReworkOrder for rejection: {str(e)}")
                            traceback.print_exc()
    except Exception as e:
